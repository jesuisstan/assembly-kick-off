...
void test_read_file(void) {
    ...
    int fd1 = open("test/test_read_file.txt", O_RDONLY);
    int fd2 = open("test/test_read_file.txt", O_RDONLY);
    if (fd1 < 0 || fd2 < 0) {
        printf("File 'test/test_read_file.txt' not found. Please add it to the test directory.\n");
        printf("-----------------------------\n");
        if (fd1 >= 0) close(fd1);
        if (fd2 >= 0) close(fd2);
        return;
